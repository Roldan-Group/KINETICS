...
def fitting(x, y):
    popt, pcov = curve_fit(logarithmic, x, y)  #, bounds=([0, 0, 0, 0], [1, 1, 1, 1]))
    return popt

#popt = fitting(x, y)
#root_squared = 1-np.sqrt(sum([(y[i] - logarithmic(x[i], *popt))**2 for i in range(len(x))])/sum(i*i for i in y))
...
#         logarithmic(np.linspace(min(x)-x_range*0.01, max(x)+x_range*0.05), *popt), "b--", lw=0.5, label=label_trend)

plt.xlabel("$E_{r}$ $(eV)$", fontsize=14)
...
for i in range(len(x)):
    plt.annotate("$Au_{"+str(i+1)+"\plus 1}$", xy=(x[i], y[i]), xycoords="data",  # size=14,
                     xytext=(x[i]-x_range*0, y[i]-y_range*0.05), textcoords="data" #,

...
close OUT;
#----------------------------------------------------------------------------------------------------- surf.x3
system ("multicell.pl $filename.surf $multiple >>kk-temp.$filename");
#----------------------------------------------------------------------------------------------------- read2
...
close OUT;
#----------------------------------------------------------------------------------------------------- cartesian
system ("cartesian.pl $filename >>kk-temp.$filename");
    $molecatoms=$natoms-$slabatoms;
system ("tail -$molecatoms $filename.cart >> $filename.x$multiple");
#system ("\$HOME/software/vasp2pov.pl $filename.x$multiple 0; mv $filename.x$multiple.x1.pov tmppov" );
#system ("sed \"s/declare ncells=1/declare ncells=$multiple/g\" tmppov >> $filename.x$multiple.x1.pov ;");
system ("rm $filename.cart $filename.surf.x$multiple $filename.surf kk-temp.$filename ");

...
#               - Differentiates between direct/indirect adsorption and mobile/immobile desorption
#
#
####################
...
#----------------------------------------------------------------------------------------------------------------------- molecule
        if (@molecules[$#molecules] eq @nsys[$nsystems]) {
            ...
            if (@tag[0] eq 'ISITES') { @tmpsites=split(/=/,$line); @tmp=split(/\s+/,@tmpsites[1]);
                ...
                if (looks_like_number(@tmp[0])) { @nsitetype{@nsys[$nsystems]}=@tmp[0]; @sitetype{@nsys[$nsystems]}=@tmp[1];
                }else{ @sitetype{@nsys[$nsystems]}=@tmp[0]; @nsitetype{@nsys[$nsystems]}=@tmp[1]; };
# not necessary with Input2mk                if (!$nsitetype{@nsys[$nsystems]}) { @nsitetype{@nsys[$nsystems]}=1.0; };
            };
...
# not necessary with Input2mk                if (!$nsitetype{@nsys[$nsystems]}) { @nsitetype{@nsys[$nsystems]}=1.0; };
            ...
            if (@tag[0] eq 'INTERPOLATE') { $t=2; @tmp=''; @interpsys{@nsys[$nsystems]}='';
                ...
                @interpsys{@nsys[$nsystems]}="@tmp"; push(@interpolated, @nsys[$nsystems]);
            };
        }; #-->if no molecule
#----------------------------------------------------------------------------------------------------------------------- end system
...
foreach $inter (@interpolated) { @nsys = grep { $_ ne $inter } @nsys; };
...
foreach $pro (@process) { @tmp=split(/\s+/,@pro); $typeproc=$tmp[0];
    if (($typeproc != 'IA') or ($typeproc != 'ia') or ($typeproc != 'DA') or ($typeproc != 'da') or     # ----- Indirect/Direct Adsorption
        ($typeproc != 'ID') or ($typeproc != 'id') or ($typeproc != 'MD') or ($typeproc != 'md') or     # ----- Immobile/Mobile Desorption
        ($typeproc != 'R') or ($typeproc != 'r')) {
...
@done=();
foreach $mol (@molecules) { $tmp=1;
    foreach $did (@done) { if ($did eq $mol) { $tmp=0; };};
...
foreach $exp (@experiments) {
    ...
    if (($exp eq "const_TEMP") or ($exp eq "variable_TEMP")) {
        ...
        foreach $i (@IniCon) {
            if ($i) { push(@Nline,$i); }; }; @IniCon=@Nline; @Nline=();
        foreach $v (@variables) {
        ...
        if ($exp eq "const_TEMP") { print "\t\t... done\n"; }else{ print "\t... done\n";};
    }elsif ($exp eq "TPR") {
        ()=&Introduction_sub($exp);
        ()=&ODE_import_sub($exp);
        @TPRmol=();
        for ($pr=1; $pr<=$#process; $pr++ ) {
            if ((@typeproc[$pr] eq "IA") or (@typeproc[$pr] eq "ia") or
                (@typeproc[$pr] eq "DA") or (@typeproc[$pr] eq "da")) {
                @PR=split(/\s+/,@ProcessReactants[$pr]);
                foreach $R (@PR) {
                    foreach $mol (@molecules) {
...
#
#=======================================================================================================================
#       MOST OF THE PARAMETERS ARE PROVIDED BY Input2mk.pl
#=======================================================================================================================
...
sub symmetry_sub {
    ...
    @stmp=split(/\s+/,$prop);
    foreach $l (@stmp) {
        if ($l) { push(@Nline,$l); }; }; @stmp=@Nline; @Nline=();
    if (@stmp[0] eq 'The') { $Gsym=@stmp[7];
    }elsif (@stmp[0] eq 'NOSYMM:'){ $Gsym=@stmp[9]; };
    system ("rm temp.prop");
    return ($GlobalSym);
}; #--> sub symmetry
#=======================================================================================================================
...
sub symfactor_sub {
    ...
    $tatoms=$#natoms+1;
    if ($linear eq "yes") {
        if ($tatoms == 1) { $symfac=2; };
        if ($tatoms >= 2) { $symfac=1; };
    }else{ @tmp=split(//,$symme);
        if (@tmp[2] == 1) {
...
sub thermo_sub {
    ...
    foreach $f (@ifreq) {
        if (($f) and (looks_like_number($f)) and ($f != 0)) { push(@Nline,$f); }; }; @ifreq=@Nline; @Nline=();
    foreach $f (@ifreq2D) {
        if (($f) or (looks_like_number($f))) { push(@Nline,$f); }; }; @ifreq2D=@Nline; @Nline=();
...
# 	     print OUT "\t\t\t\t\tfprintf(fileID, \' linear molecule = @linear{$sys}\\n\');\n";
        ...
        print OUT "  area$sys=@Acat{@sitetype{$sys}}*Nsites$sys;\t\tfprintf(fileID, \' area on the surface = %.15E m^2\\n\',area$sys);\n";
        ...
        print OUT "    qtrans2D$sys=((area$sys)*2*pi*Mass$sys*kb*T)/(h^2);	% free molecule moving in 2 dimensions (hovering on surface)\n";
            ...
            while ( $tmp <= $nqvib ) {
                ...
                for ($k=50*$tmp; $k<=49+50*$tmp; $k++) {
                    if (($k < 49+50*$tmp) and ( $nf< $#ifreq)) { print OUT "1/(1-exp(-h*c*$ifreq[$k]/(kb*T)))*"; $nf++;
                        ...
                        if (@ifreq[$k]) { print OUT "1/(1-exp(-h*c*$ifreq[$k]/(kb*T)));\n"; $nf++; };};
                }; $tmp++; };
...
# re-Formulation from explicit derivatives:: https://wiki.fysik.dtu.dk/ase/ase/thermochemistry/thermochemistry.html
    ...
    }else{ $nf=0; print OUT "kb*(";
        foreach $f (@ifreq) {
            if ($nf < $#ifreq) { print OUT "(h*c*$f)/(kb*T*(exp(h*c*$f/(kb*T)))) - log(1-exp(-h*c*$f/(kb*T)))+"; $nf++;
            }else{ print OUT "(h*c*$f)/(kb*T*(exp(h*c*$f/(kb*T)))) - log(1-exp(-h*c*$f/(kb*T))) )*JtoeV;\n"; $nf++; };};
    };
...
#           print OUT "    H0$sys=@e0{$sys}+ZPE$sys+int(Cp$sys,T);\n";  push(@variable,"H0$sys");
    ...
    if ($sys eq "O2") {
        print OUT "% H$sys=$e0{$sys}+0.9/2+MZPE$sys(i,2)+integral(funcH$sys,0,T,'RelTol', 1e-3, 'AbsTol', 1e-4); % O2 overbinding correction (O2=-5.17eV)\n";
        push(@variable,"H$sys");
    }else{
        print OUT "% H$sys=$e0{$sys}+MZPE$sys(i,2)+integral(funcH$sys,0,T,'RelTol', 1e-3, 'AbsTol', 1e-4);\n";
        push(@variable,"H$sys");};
    print OUT "% G$sys=H$sys-T*S$sys;\n\n";     push(@variable,"G$sys");
#-----------------------------------------------------------------------------------------------------------------------
    if ($ttemp) { $size=($ftemp-$itemp)/$ttemp;
    }else{ $size=1; };
    ...
    if ($sys eq "O2") { print OUT "  MH$sys(i,:)=[T, subs($e0{$sys}+0.9/2+MZPE$sys(i,2)+integral(funcH$sys,0,T,'RelTol', 1e-3, 'AbsTol', 1e-4))];\n";
        ...
        if (!$ifreq[0]) {
            ...
            print OUT "  MH$sys(i,:)=[T, subs($e0{$sys}+MZPE$sys(i,2)+integral(funcH$sys,0,T,'RelTol', 1e-3, 'AbsTol', 1e-4))];\n"; };};
...
#----------------------------------------------------------------------------------------------------------------------- stoichiometry
    ...
    foreach $cha (@tmp) {
        if ($cha) {
            if (($step ne '>') and ($step ne '>>')) {  $step=$cha;
                if (($cha ne '+') and ($cha ne '>') and ($step ne '>>')){ $stoichio{@PR[$r]}->[$pr]=$cha; $r++; };
            }else{
    ...
    foreach $r (@PR) {
        printf OUT2 "%3s*%s ",$stoichio{$r}->[$pr],$r; $processWidth=$processWidth+max(map length, $r)+3; };
    print OUT2 colored(">","bold red");
    foreach $ts (@PTS) {
        printf OUT2 colored(" $ts ","bright_yellow"); $processWidth=$processWidth+max(map length, $ts)+3; };
    print OUT2 colored(">","bold green");
    foreach $p (@PP) {
        printf OUT2 " $stoichio{$p}->[$pr]*%-s",$p; $processWidth=$processWidth+max(map length, $p)+3; };
    printf OUT2 "%-*s\t",40-($processWidth);
    $rE=0; $tE=(); $pE=0;
    foreach $r (@PR) {
        printf OUT2 "%.2f ",$e0{$r}*$stoichio{$r}->[$pr]; $rE=$rE+$e0{$r}*$stoichio{$r}->[$pr]; };
    print OUT2 colored("> ","bold red");
    foreach $ts (@PTS) { printf OUT2 "%.2f ",$e0{$ts}; $tE=$tE+$e0{$ts};};
    print OUT2 colored("> ","bold green");
    foreach $p (@PP) { printf OUT2 "%.2f ",$e0{$p}*$stoichio{$p}->[$pr]; $pE=$pE+$e0{$p}*$stoichio{$p}->[$pr]; };
    $nS=$#PR+$#PTS+$#PP+3;
    if ($nS eq 6) { printf OUT2 " ";
    }elsif ($nS eq 5) { printf OUT2 "\t%5s";
    }elsif ($nS eq 4) { printf OUT2 "\t\t%5s";
    }elsif ($nS eq 3) { printf OUT2 "\t\t\t%5s";
    }elsif ($nS eq 2) { printf OUT2 "\t\t\t\t%5s"; };
    if ($tE) { print OUT2 "Ea=";
    ...
    $rE=0; $tE=(); $pE=0;
    foreach $r (@PR) { 	printf OUT3 "%.2f ",$e0{$r}*$stoichio{$r}->[$pr]; 	$rE=$rE+$e0{$r}*$stoichio{$r}->[$pr]; };
    print OUT3 "> ";
    foreach $ts (@PTS) { 	printf OUT3 "%.2f ",$e0{$ts}; 				$tE=$tE+$e0{$ts}; };
    print OUT3 "> ";
    foreach $p (@PP) { 	printf OUT3 "%.2f ",$e0{$p}*$stoichio{$p}->[$pr]; 	$pE=$pE+$e0{$p}*$stoichio{$p}->[$pr]; };
    $nS=$#PR+$#PTS+$#PP+3;
    if ($nS eq 6) { print OUT3 " ";
    }elsif ($nS eq 5) { printf OUT3 "\t%5s";
    }elsif ($nS eq 4) { printf OUT3 "\t\t%5s";
    }elsif ($nS eq 3) { printf OUT3 "\t\t\t%5s";
    }elsif ($nS eq 2) { printf OUT3 "\t\t\t\t%5s"; };
    if ($tE) { printf OUT3 "Ea=%.2f eV ",$tE-$rE;
    }else{ printf OUT3 "%11s";};
...
#    print OUT "\n";
    ...
    if (@PTS) {
        ...
        if (($typeP eq 'DA') or ($typeP eq 'da')) {
            $comment = "DIRECT ADSORPTION: Volumetric qtrans does not contribute to E";
            foreach $R (@PR) {
                ...
                if (@en{$R}) {push(@Etmp2, "+stoichio$pr$R*(@en{$R}-(3/2)*kb*T)");
                }else{
                    push(@Etmp2, "+stoichio$pr$R*(E$R-(3/2)*kb*T)");
                    push(@Esyms, "E$R");
                };
            };
        }elsif (($typeP eq 'IA') or ($typeP eq 'ia')) {
            $comment = "INDIRECT ADSORPTION: Only 2D qtrans contributes to E";
            foreach $R (@PR) {
                if (!grep("E$R", @Esyms)) {push(@Esyms, "E$R");};
                foreach $interp (@interpolated) {
                    if ($interp eq $R) {
                ...
                if (@en{$R}) {push(@Etmp2, "+stoichio$pr$R*(@en{$R}-(1/2)*kb*T)");
                }else{
                    push(@Etmp2, "+stoichio$pr$R*(E$R-(1/2)*kb*T)");
                    push(@Esyms, "E$R");
                };
            };
        }elsif (($typeP eq 'MD') or ($typeP eq 'md')) {
             $comment="MOBILE DESORPTION: Only 2D qtrans contributes to E";
             foreach $P (@PP) { push(@Esyms,"E$P");
                 foreach $interp (@interpolated) {
                    if ($interp eq $P) {  @tmp=split(/\s+/,@interpsys{$P}); print OUT "%   $interp=@tmp[2];\n"; };};
                 print OUT "\n";
                 if (@en{$P}) { push(@Etmp2,"+stoichio$pr$P*(@en{$P}-(1/2)*kb*T)");
                 }else{ push(@Etmp2,"+stoichio$pr$P*(E$P-(1/2)*kb*T)");};};
        }elsif (($typeP eq 'ID') or ($typeP eq 'id')) {
             $comment="IMMOBILE DESORPTION: Volumetric qtrans does not contribute to E";
             foreach $P (@PP) { push(@Esyms,"E$P");
                 foreach $interp (@interpolated) {
                    if ($interp eq $P) {  @tmp=split(/\s+/,@interpsys{$P}); print OUT "%   $interp=@tmp[2];\n"; };};
                 print OUT "\n";
                 if (@en{$P}) { push(@Etmp2,"+stoichio$pr$P*(@en{$P}-(3/2)*kb*T)");
                 }else{ push(@Etmp2,"+stoichio$pr$P*(E$P-(3/2)*kb*T)");};};
        }else{
...
sub ProcessQ_sub {
    ...
    #                       for the different A and D degrees of freedom
        ...
        if (($typeP eq 'IA') or ($typeP eq 'ia')) {
                ...
                if (@q{$R}) {
                    ...
                    foreach $mol (@molecules) {
                        ...
                        if ($go eq 'yes') {
                            ...
                            push(@Qtmp2,"*Q3D$R^stoichio$pr$R"); push(@Qsyms,"Q3D$R");};};};
            ...
            foreach $R (@PR) { $go='no';
                foreach $mol (@molecules) {
                    if ($R eq $mol) { $go='yes'; }; };
                if ($go eq 'yes') { push(@Qtmp2,"*(qvib3D$R*qtrans2D$R*qrot$R)^stoichio$pr$R");
                    push(@Qsyms,"vib3D$R qtrans2D$R qrot$R"); };
            };
            foreach $P (@PP) { $go='no';
                foreach $mol (@molecules) {
                    if ($P eq $mol) {  $go='yes'; }; };
                if ($go eq 'yes') { push(@Qtmp2,"*(qvib3D$P*qtrans2D$P*qrot$P)^stoichio$pr$P");
                    push(@Qsyms,"vib3D$P qtrans2D$P qrot$P");
                }else{
                    if (@q{$P}) { push(@Qtmp2,"@q{$P}^stoichio$pr$P");
                        foreach $interp (@interpolated) {
                            if ($interp eq $P) { push(@Qsyms,"Q3D$P"); @tmp=split(/\s+/,@interpsys{$P});
    ...
    open OUT, ">>processes.m";
    print OUT "syms";
    foreach $s (@Qsyms) { print OUT " $s"; }; print OUT "\t\t\t\% $comment\n";
...
#	}else{ @r=(); $tmp=(); 
    ...
    if (($typeP eq 'IA') or ($typeP eq 'ia') or ($typeP eq 'DA') or ($typeP eq 'da')) {
        ...
        print OUT "Krate$pr=sticky$pr*Arrhenius$pr;\n";
    }else{
...
sub DRCrates_sub {
    ...
    open OUT, ">>$exp.m"; %done=();
    print OUT "\nsyms ";
        for ($pr=1; $pr<=$#process; $pr++ ) { print OUT " Krate$pr"; }; print OUT "\n";
        print OUT "syms ";
        for ($pr=1; $pr<=$#process; $pr++ ) { @PR=split(/\s+/,@ProcessReactants[$pr]);
            foreach $R (@PR) {
                if (!$done{$R}) {
                    foreach $mol (@molecules) {
                        if ($R eq $mol) {  print OUT " P$R"; };};
                    foreach $sur (@surfaces) {
                        if ($R eq $sur) {   print OUT " coverage$R"; };};
                    foreach $cat (@catalysts) {
                        if ($R eq $cat) {  print OUT " coverage$R"; };};
                    $done{$R}=1; };};}; print OUT "\n";
        for ($pr=1; $pr<=$#process; $pr++ ) { @DRCr=(); $tmp=(); $DRCnrate="Krate$pr";
            @PR=split(/\s+/,@ProcessReactants[$pr]);
            foreach $R (@PR) {
                foreach $mol (@molecules) {
                    if ($R eq $mol) {  push(@DRCr,"P$mol^stoichio$pr$mol"); $tmp=$mol;};};
                foreach $sur (@surfaces) {
                    if ($R eq $sur) {   push(@DRCr,"coverage$sur^stoichio$pr$sur"); };};
                foreach $cat (@catalysts) {
                    if ($R eq $cat) {  push(@DRCr,"coverage$cat^stoichio$pr$cat"); };};};
    ...
    foreach $R (@PR) {
        if (@do{$R} eq "yes") { $qmol="no";
            ...
            if ($qmol eq "yes") {   print OUT "      E$R=ENERGY$R\{j,2}; Z$R=ZPE$R\{j,2}; Z2D$R=ZPE2D$R\{j,2};\n";
                print OUT "      Q3D$R=PARTITION3D$R\{j,2}; qtrans2D$R=qt2d$R\{j,2}; qtrans3D$R=qt3d$R\{j,2};";
                print OUT " qvib2D$R=qv2d$R\{j,2}; qvib3D$R=qv3d$R\{j,2}; qrot$R=qr$R\{j,2};\n"; @do{$R}="no";
            }else{
# 31/05/2021
                foreach $interp (@interpolated) {
    ...
    foreach $P (@PP) {
        if (@do{$P} eq "yes") { $qmol="no";
            ...
            if ($qmol eq "yes") {    print OUT "      E$P=ENERGY$P\{j,2}; Z$P=ZPE$P\{j,2}; Z2D$P=ZPE2D$P\{j,2};\n";
                print OUT "      Q3D$P=PARTITION3D$P\{j,2}; qtrans2D$P=qt2d$P\{j,2}; qtrans3D$P=qt3d$P\{j,2};";
                print OUT " qvib2D$P=qv2d$P\{j,2}; qvib3D$P=qv3d$P\{j,2}; qrot$P=qr$P\{j,2};\n"; @do{$P}="no";
            }else{
                foreach $interp (@interpolated) {
...
# Now setting and printing the initial conditions
	if ($exp eq "TPR") {
        ...
        $TPRdone="n";
        for ($pr=1; $pr<=$#process; $pr++ ) {
            if ((@typeproc[$pr] eq "DA") or (@typeproc[$pr] eq "da")
                or (@typeproc[$pr] eq "IA") or (@typeproc[$pr] eq "ia")) {
                @PR=split(/\s+/,@ProcessReactants[$pr]);
                foreach $R (@PR) {
...
#----------------------------------------------------------------------------------------------------------------------- appear / desappear
    for ($pr=1; $pr<=$#process; $pr++ ) {
        ...
        @PP=split(/\s+/,@ProcessProducts[$pr]);
        if (($typeproc[$pr] eq 'DA') or ($typeproc[$pr] eq 'da')
            or (@typeproc[$pr] eq "IA") or (@typeproc[$pr] eq "ia")) { $tmpmol=(); $tmpcat=();
            if ($exp ne "TPR") {
                foreach $Rname (@PR) {
                    foreach $mol (@molecules) {
        ...
        }elsif (($typeproc[$pr] eq 'MD') or ($typeproc[$pr] eq 'md')
            or (@typeproc[$pr] eq "ID") or (@typeproc[$pr] eq "id")) { $tmpcat=();
            foreach $Rname (@PR) {
                foreach $mol (@molecules) {
                    if ($Rname eq $mol) {
                        if (!$Desequation{$mol}) {
   ...
   sub DRC_call_sub {
	...
	for ($pr=1; $pr<=$#process; $pr++ ) { if (!$done[$pr]) {
		...
		for ($i=1; $i<=$#process; $i++ ) { if (($i eq $pr) or ($i eq $reverse[$pr])) {
			if (($typeproc[$i] eq "DA") or ($typeproc[$i] eq "da")
                or (@typeproc[$pr] eq "IA") or (@typeproc[$pr] eq "ia")) {
			...
			if (($typeproc[$i] eq "DA") or ($typeproc[$reverse[$i]] eq "da")
                or (@typeproc[$pr] eq "IA") or (@typeproc[$pr] eq "ia")) {
